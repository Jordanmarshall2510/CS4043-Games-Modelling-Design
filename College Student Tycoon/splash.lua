...
function splash:show( event )
    ...
    if ( phase == "will" ) then
	display.newImage( sceneGroup, "crest.png", display.contentCenterX, display.contentCenterY , native.systemFont , 60 )

	elseif ( phase == "did" ) then 
	composer.removeScene( "splash" ) 
	composer.gotoScene( "title", {effect = "fade", time = 4000} )
	end
